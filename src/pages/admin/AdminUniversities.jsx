...
import { getUniversitiesResolved, saveUniversitiesOverride, getCountriesResolved, subscribeSiteData } from '../../lib/siteData.js';
import { supabase } from '../../lib/supabase.js';
import { compressImage } from '../../lib/compressImage.js';
import { useSiteCountries } from '../../hooks/useSiteCountries.js';

function programsToString(programs) {
...
export default function AdminUniversities() {
  const [items, setItems] = useState(() => getUniversitiesResolved());

  useEffect(() => {
    return subscribeSiteData(() => {
      setItems(getUniversitiesResolved());
    });
  }, []);
  ...
  const availableCountries = useMemo(() => siteCountries.map((c) => c.name), [siteCountries]);
  ...
  function handleEditItem(item) {
    ...
    setFormData({
      ...item,
      programs: programsToString(item.programs),
    });
    setView('form');
  ...
  function handleSaveForm(e) {
    ...
    const selectedCountry = formData.country || (availableCountries.length > 0 ? availableCountries[0] : '');
    
    const newRecord = {
      ...formData,
      country: selectedCountry,
      programs: parsePrograms(formData.programs),
    ...
    setItems(nextItems);
    saveUniversitiesOverride(nextItems);
      ...
      {view === 'list' ? (
        <div className="animate-fade-in">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
            <h1 className="text-2xl font-black text-slate-950 dark:text-white">Universitetlar</h1>
            <Button onClick={handleAddNew}>+ Yangi qo'shish</Button>
          </div>

          <input
            type="text"
            placeholder="Qidirish (nomi, davlati)..."
            value={searchTerm}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{row.country} • {row.city}</p>
                  </div>
                </div>
        ...
        <div className="animate-fade-in max-w-3xl mx-auto rounded-3xl border border-slate-200 bg-white p-6 sm:p-10 shadow-sm dark:border-white/10 dark:bg-slate-900">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">
              {isEditing ? "Universitetni Tahrirlash" : "Yangi Universitet Qo'shish"}
            </h2>
          ...
          <form onSubmit={handleSaveForm} className="grid gap-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <label className="grid gap-1 text-xs font-bold text-slate-600 dark:text-slate-400">
                ...
                <input
                  ...
                  onChange={(e) => handleFormChange('ranking', e.target.value)}
                  className="rounded-full border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm outline-none focus:border-brand-500 focus:bg-white dark:border-white/10 dark:bg-slate-950 dark:text-white"
                />
              </label>
            </div>
            ...
            <label className="grid gap-1 text-xs font-bold text-slate-600 dark:text-slate-400">
              ...
              <input
                ...
                onChange={(e) => handleFormChange('tuition', e.target.value)}
                placeholder="From €2,000/year"
                className="rounded-full border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm outline-none focus:border-brand-500 focus:bg-white dark:border-white/10 dark:bg-slate-950 dark:text-white"
              />
            </label>

            <label className="grid gap-1 text-xs font-bold text-slate-600 dark:text-slate-400">
              Dasturlar (vergul bilan ajrating)
              <input
                value={formData.programs}
                onChange={(e) => handleFormChange('programs', e.target.value)}
                placeholder="IT, Business, Medicine"
                className="rounded-full border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm outline-none focus:border-brand-500 focus:bg-white dark:border-white/10 dark:bg-slate-950 dark:text-white"
              />
            </label>
